...
class Resolver
{
    ...
    public function relocate(array $templates): array
    {
        ...
        $templates = array_map(fn($item) => bt()->config()->get('views.path') . '/' . $item . '.blade.php', $templates);
        $templates = array_map(fn($item) => str_replace(bt()->config()->get('resources.path') . '/', '', $item), $templates);
    ...
    public function render(string $template): string
    {
        ...
        return bt()->config()->get('resources.path') . '/index.php';
    }
}
